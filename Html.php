...
HTML;
    ...
    public static function includeHeader(
       ...
       // load log filters everywhere
        ...
        // Fix para dropdown de catálogo - Select2 (solo para tema idtheme)
        if ($theme === 'idtheme') {
            $tpl_vars['js_files'][] = ['path' => 'js/select2_catalog_fix.js'];
        }
       ...
       // Apply all GLPI styles to editor content
        ...
        $js = <<<JS
         $(function() {
            ...
            // Ocultar el textarea inmediatamente antes de inicializar el editor
            var textareaElement = $('#$id');
            textareaElement.css({
            ...
            // Observar cambios en el estilo del textarea para mantenerlo oculto
            ...
            observer.observe(textareaElement[0], { attributes: true, attributeFilter: ['style'] });
            ...
            tinyMCE.init(Object.assign({
               ...
               setup: function(editor) {
                  ...
                  editor.on('init', function (e) {
                     // Ocultar completamente el textarea cuando el editor está activo para evitar mostrar HTML crudo
                     var textarea = $('#$id');
                     textarea.css({
                        'display': 'none !important',
                        'visibility': 'hidden',
                     ...
                     // También ocultar cualquier campo de descripción relacionado que pueda estar mostrando el HTML
                     textarea.closest('.form-field').find('input[name="content"], textarea[name="content"]').not('#$id').each(function() {
                        $(this).css({
                           'display': 'none !important',
                           'visibility': 'hidden'
                        });
                     ...
                     // Eliminar el borde rojo estático del contenedor del editor cuando tiene clase "required"
                     var editorContainer = $(editor.container);
                     // Aplicar borde gris inicial igual que los inputs normales
                     editorContainer.css({
                     ...
                     // Manejar el foco dinámicamente para mostrar/ocultar el borde rojo
                     editor.on('focus', function() {
                        editorContainer.css({
                           'border': '1px solid #c8202f',
                           'border-color': '#c8202f',
                           'border-left-color': '#c8202f',
                           'border-left-width': '1px',
                           'box-shadow': '0 0 0 0.2rem rgba(200, 32, 47, 0.15)'
                        }).addClass('tox-focused');
                     });
                     
                     editor.on('blur', function() {
                        // Volver al color gris exacto igual que los inputs normales
                        editorContainer.css({
                           'border': '1px solid #e0e0e0',
                           'border-color': '#e0e0e0',
                           'border-left-color': '#e0e0e0',
                           'border-left-width': '1px',
                           'box-shadow': 'none'
                        }).removeClass('tox-focused');
                     });
                     
                     // También manejar cuando se hace clic dentro del iframe
                     setTimeout(function() {
                        var iframe = editor.iframeElement;
                        if (iframe && iframe.contentDocument) {
                           var iframeBody = iframe.contentDocument.body;
                           if (iframeBody) {
                              // Inyectar estilos directamente en el iframe para eliminar el borde rojo interno
                              var style = iframe.contentDocument.createElement('style');
                              style.textContent = 'body.mce-content-body, body#tinymce.mce-content-body, body.mce-content-body.rich_text_container { outline: none !important; border: none !important; box-shadow: none !important; } body.mce-content-body:focus, body.mce-content-body:focus-within, body.mce-content-body:active, body.mce-content-body[contenteditable="true"]:focus, body.mce-content-body[contenteditable="true"]:focus-within, body.mce-content-body[contenteditable="true"]:active { outline: none !important; border: none !important; box-shadow: none !important; }';
                              iframe.contentDocument.head.appendChild(style);
                              
                              // También eliminar el outline cuando se hace clic
                              iframeBody.addEventListener('focus', function() {
                                 iframeBody.style.outline = 'none';
                                 iframeBody.style.border = 'none';
                                 iframeBody.style.boxShadow = 'none';
                                 // Asegurar que el contenedor externo muestre el borde rojo
                                 editorContainer.css({
                                    'border': '1px solid #c8202f',
                                    'border-color': '#c8202f',
                                    'border-left-color': '#c8202f',
                              ...
                              iframeBody.addEventListener('focusin', function() {
                                 iframeBody.style.outline = 'none';
                                 iframeBody.style.border = 'none';
                                 iframeBody.style.boxShadow = 'none';
                                 editorContainer.css({
         ...
         const select2_el = $('#$id').select2({
            ...
            matcher: function(params, data) {
               ...
               if (typeof data.children === 'undefined') {
                  ...
                  data.rendered_text = match.rendered;
...
HTML;
    ...
    public static function textarea($options = [])
    {
       ...
       //merge default options with options parameter
        ...
        $display = '';
        // Ocultar completamente el textarea cuando hay editor de texto enriquecido para evitar mostrar HTML crudo
        $textarea_class = 'form-control';
        $textarea_style = '';
        $textarea_aria = '';
        if ($p['enable_richtext']) {
            $textarea_class .= ' richtext-textarea-hidden';
            $textarea_style = ' style="display: none !important; visibility: hidden; position: absolute; width: 1px; height: 1px; opacity: 0; pointer-events: none;"';
            $textarea_aria = ' aria-hidden="true"';
        }
        $display .= "<textarea class='" . $textarea_class . "' name='" . $p['name'] . "' id='" . $p['editor_id'] . "'
                             rows='" . $p['rows'] . "' cols='" . $p['cols'] . "' $required" . $textarea_style . $textarea_aria . ">" .
                  htmlspecialchars($p['value'], ENT_QUOTES, 'UTF-8') . "</textarea>";
