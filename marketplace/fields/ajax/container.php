...
if (isset($_GET['action']) && $_GET['action'] === 'get_fields_html') {
    ...
    $ids_raw = $_GET['id'] ?? '';
    $ids     = array_map('intval', array_filter(explode(',', (string) $ids_raw)));
    if (count($ids) === 0) {
    ...
    foreach ($ids as $containers_id) {
        $right = PluginFieldsProfile::getRightOnContainer($_SESSION['glpiactiveprofile']['id'], $containers_id);
        if ($right < READ) {
            continue;
        }
