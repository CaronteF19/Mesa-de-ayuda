...
class PluginFieldsField extends CommonDBChild
{
    ...
    public static function showForTab($params)
    {
        ...
        // Find all containers for this itemtype/type/subtype (permite varios bloques)
        if (isset($_REQUEST['c_id'])) {
            $c_ids = [(int) $_REQUEST['c_id']];
        } else {
            $c_ids = PluginFieldsContainer::findContainers(get_class($item), $type, $subtype);
        }
        if (count($c_ids) === 0) {
            return;
        }
        ...
        if (strpos($current_url, 'helpdesk.public.php') !== false) {
            echo "<div id='{$html_id}' class='card-body row mx-0' style='border-top:0' data-container-ids='" . implode(',', $c_ids) . "'>";
            echo "<div class='offset-md-1 col-md-8 col-xxl-6'>";
        } else {
            echo "<div id='{$html_id}' data-container-ids='" . implode(',', $c_ids) . "'>";
        }
        ...
        echo Html::scriptBlock(
            ...
            function refreshContainer() {
                ...
                var containerIds = $('#{$html_id}').attr('data-container-ids') || '';
                $.ajax(
                    {
                        url: '{$ajax_url}',
                        type: 'GET',
                ...
                function () {
                    ...
                    form.on(
                        ...
                        function(evt) {
                            ...
                            // Si el cambio es dentro del contenedor, solo refrescar si es un campo del plugin
                            // (p. ej. Propósito SGC) que puede afectar condiciones de otros bloques.
                            // Evita refrescos en cada cambio de cualquier campo y mejora rendimiento.
                            if ($(evt.target).closest('#{$html_id}').length > 0) {
                                if (!evt.target.name || evt.target.name.indexOf('plugin_fields_') !== 0) {
                                    return;
                                }
                            }
                            refreshContainer();
