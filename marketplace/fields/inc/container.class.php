...
class PluginFieldsContainer extends CommonDBTM
{
    ...
    public function prepareInputForAdd($input)
    {
        ...
        // Permitir varios bloques del mismo tipo (dom/domtab) para el mismo tipo de elemento;
        // cada bloque puede tener condiciones de visualización distintas (p. ej. por categoría).
...
HTML;
    ...
    /**
     * Find all container IDs for the given itemtype, type and subtype.
     * Used to support multiple blocks (e.g. dom) for the same element type.
     *
     * @param string $itemtype Item type (e.g. Ticket::class)
     * @param string $type     Container type ('dom', 'domtab', 'tab', etc.)
     * @param string $subtype  Subtype for domtab
     *
     * @return array List of container IDs (empty if none)
     */
    public static function findContainers($itemtype, $type = 'tab', $subtype = '')
    {
        $condition = [
            'is_active' => 1,
            ['type'     => $type],
        ...
        foreach ($itemtypes as $data) {
            $dataitemtypes = json_decode($data['itemtypes']);
            if (is_array($dataitemtypes) && in_array($itemtype, $dataitemtypes)) {
                $cid = (int) $data['id'];
                if ($profile_id !== null && $cid > 0) {
                    $right = PluginFieldsProfile::getRightOnContainer($profile_id, $cid);
                    if ($right < READ) {
                        continue;
                    }
                }
    ...
    public static function postItemAdd(CommonDBTM $item)
    {
        ...
        $list = isset($data['plugin_fields_containers_id']) ? [$data] : $data;
        $container = new self();
        foreach ($list as $one) {
            $one['items_id']     = $item->getID();
            $one['entities_id']  = $item->isEntityAssign() ? $item->getEntityID() : 0;
            if (!$container->updateFieldsValues($one, $item->getType(), isset($_REQUEST['massiveaction']))) {
                $item->input = [];
                return $item;
            }
        }
    ...
    public static function preItemUpdate(CommonDBTM $item)
    {
        ...
        foreach ($list as $one) {
            $one['entities_id'] = $item->isEntityAssign() ? $item->getEntityID() : 0;
            if (count($one) > 0 && !$container->updateFieldsValues($one, $item->getType(), isset($_REQUEST['massiveaction']))) {
                return false;
            }
        }
    ...
    public static function preItem(CommonDBTM $item)
    {
        ...
        if (isset($item->input['c_id'])) {
            $c_ids = [(int) $item->input['c_id']];
        } elseif (isset($_REQUEST['c_id'])) {
            $c_ids = [(int) $_REQUEST['c_id']];
        } else {
            $type = 'dom';
            ...
            $c_ids = self::findContainers(get_class($item), $type, $subtype);
            if (count($c_ids) === 0) {
                $tab_ids = self::findContainers(get_class($item));
                if (count($tab_ids) === 0) {
                    return false;
                }
                $c_ids = $tab_ids;
        ...
        foreach ($c_ids as $c_id) {
            ...
            if ($data !== false && self::validateValues($data, $item::getType(), isset($_REQUEST['massiveaction'])) !== false) {
                $all_data[] = $data;
            }
        }
        ...
        $item->input['_plugin_fields_data'] = count($all_data) === 1 ? $all_data[0] : $all_data;
