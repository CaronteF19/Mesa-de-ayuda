...
var query = {};
function stripDropdownChevrons(str) {
    if (!str || typeof str !== 'string') return str;
    // Quitar >>, », y separadores " > " que vienen del árbol (getTreeValueName, completename)
    str = str.replace(/>>/g, ' ').replace(/\s*>\s*>\s*/g, ' ');
    str = str.replace(/\s*>\s*>/g, ' ').replace(/>\s*>\s*/g, ' ');
    str = str.replace(/[\u00BB\u203A\u226A\u226B]/g, ''); // » › ≪ ≫
    str = str.replace(/\s*>\s*/g, ' '); // un solo " > " como separador (p. ej. "Padre > Hijo")
    return str.replace(/^[\s\u00A0]*(>>|»»|»|\u00BB|\u203A|\u226A|\u226B)+\s*/g, '').replace(/\s*(>>|»»|»|\u00BB|\u203A|\u226A|\u226B)\s*$/g, '').trim();
}
/**
 * Function that renders select2 results.
 * Solo muestra el texto filtrado, sin subrayar coincidencias ni ">>".
 */
var templateResult = function(result) {
    var _elt = $('<span></span>');
    _elt.attr('title', result.title ? stripDropdownChevrons(result.title) : '');

    if (typeof query.term !== 'undefined' && typeof result.rendered_text !== 'undefined') {
        var rt = stripDropdownChevrons(result.rendered_text || '');
        rt = rt.replace(/<span[^>]*class="[^"]*select2-rendered__match[^"]*"[^>]*>([\s\S]*?)<\/span>/gi, '$1');
        rt = rt.replace(/<span class='select2-rendered__match'>([\s\S]*?)<\/span>/gi, '$1');
        _elt.html(rt);
    } else {
        if (!result.text) {
        ...
        var text = stripDropdownChevrons(result.text);
        if (!result.id) {
            _elt.html(escapeMarkupText(text));
            return _elt;
        }

        // Mostrar solo texto, sin resaltar coincidencias (sin markMatch / select2-rendered__match)
        if (result.level) {
            var marginPx = (result.level - 1) * 12;
            _elt.html('<span style="display:inline-block;margin-left:' + marginPx + 'px">' + escapeMarkupText(text) + '</span>');
        } else {
            _elt.html(escapeMarkupText(text));
        }
    }
