...
class TemplateRenderer
{
    ...
    public function __construct(string $rootdir = GLPI_ROOT, string $cachedir = GLPI_CACHE_DIR)
    {
        ...
        // Asegura que exista el directorio de caché; si no se puede crear
        // o no es escribible, Twig funcionará sin caché en disco y sin warning.
        if (!file_exists($tpl_cachedir)) {
            @mkdir($tpl_cachedir, 0777, true);
        }

        if (is_dir($tpl_cachedir) && is_writable($tpl_cachedir)) {
            $env_params['cache'] = $tpl_cachedir;
        } else {
            // En algunos entornos Windows/XAMPP is_writable() puede fallar
            // aunque el servidor pueda seguir renderizando sin problema.
            // Si quieres, puedes dejar un log silencioso:
            // error_log(sprintf('GLPI: cache directory \"%s\" es no escribible.', $tpl_cachedir));
        }
