...
class StatDataTicketNumber extends StatDataAlwaysDisplay
{
    public function __construct(array $params)
    {
        ...
        // Traducir los nombres de las series - forzar traducción al español
        $opened_name = _nx('ticket', 'Opened', 'Opened', Session::getPluralNumber());
        $solved_name = _nx('ticket', 'Solved', 'Solved', Session::getPluralNumber());
        
        // Si la traducción no se aplicó (por ejemplo, si el archivo .mo no está compilado),
        // usar traducciones directas en español basadas en el idioma de la sesión
        $current_lang = Session::getCurrentLanguage();
        if (strpos($current_lang, 'es') === 0) {
            // Español (Colombia, Argentina, etc.) - usar singular para leyendas de gráficos
            if ($opened_name === 'Opened') {
                $opened_name = 'Abierto';
            }
            if ($solved_name === 'Solved') {
                $solved_name = 'Resuelto';
            }
        }
