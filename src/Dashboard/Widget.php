...
HTML;
      ...
      $(function () {
         ...
         chart.on('draw', function(data) {
            ...
            // Traducir textos de la leyenda después de que se cree el gráfico
         var traducirLeyenda = function() {
            ...
            // Buscar y traducir elementos de la leyenda
            var legendItems = chart.container.querySelectorAll('.ct-legend li');
            legendItems.forEach(function(item) {
               var texto = item.textContent.trim();
               if (traducciones[texto]) {
                  item.textContent = traducciones[texto];
               }
            });
         };
         ...
         chart.on('created', function(bar) {
            ...
            traducirLeyenda();
            // También traducir después de un pequeño delay por si la leyenda se crea después
            setTimeout(traducirLeyenda, 100);
            setTimeout(traducirLeyenda, 500);
         });
...
HTML;
      ...
      $(function () {
         ...
         // Traducir tooltips de forma segura después de que el gráfico esté completamente creado
         setTimeout(function() {
            var traducirTooltips = function() {
               var tooltips = document.querySelectorAll('.dashboard-tooltip');
               tooltips.forEach(function(tooltip) {
                  if (tooltip) {
                     // Traducir el texto dentro del span de meta (nombre de la serie)
                     // Esto preserva el formato HTML con <br> entre nombre y valor
                     var metaSpan = tooltip.querySelector('.chartist-tooltip-meta');
                     if (metaSpan) {
                        var textoMeta = metaSpan.textContent || metaSpan.innerText || '';
                        if (textoMeta.trim() === 'Opened') {
                           metaSpan.textContent = 'Abierto';
                        } else if (textoMeta.trim() === 'Solved') {
                           metaSpan.textContent = 'Resuelto';
                        }
                     }
                     
                     // También verificar si hay texto sin spans (fallback)
                     var innerHTML = tooltip.innerHTML || '';
                     if (innerHTML.indexOf('Opened') !== -1 || innerHTML.indexOf('Solved') !== -1) {
                        // Reemplazar en el HTML preservando la estructura
                        innerHTML = innerHTML.replace(/>Opened</g, '>Abierto<');
                        innerHTML = innerHTML.replace(/>Solved</g, '>Resuelto<');
            ...
            // Traducir cuando aparezcan tooltips (usando eventos del DOM)
            document.addEventListener('mouseover', function(e) {
               setTimeout(traducirTooltips, 10);
            }, true);
         }, 1000);
